...
(define directives
  `((#\~ . ,(lambda (d pad p) (display #\~ p)))
    ...
    ;; SRFI 19 gives "5.2" as the example for ~f, so the fraction carries only
    ;; as many digits as it needs and is dropped altogether when it is zero
    (#\f . ,(lambda (d pad p)
              (display (int-str (date-second d)) p)
              (let* ((digits (padding (date-nanosecond d) #\0 9))
                     (end (let loop ((i (string-length digits)))
                            (if (and (> i 0) (char=? (string-ref digits (- i 1)) #\0))
                                (loop (- i 1))
                                i))))
                (when (> end 0)
                  (display locale-number-separator p)
                  (display (string-copy digits 0 end) p)))))
