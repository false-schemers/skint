/* Unicode tables generator for Skint */
...
/* properties and case */
/* R7RS extras:
 * id initial:  L*, Mn, Nl, No, Pd, Pc, Po, S*, Co, U+200C, U+200D
 * id subsequent: id initial, Nd, Mc, Me
 */
...
static int process_prop(int cp, char *cat, char *dv, char *upper, char *lower, char *title, int ti, int *plti, int *puti, int *ptti)
{
  ...
  if (!strcmp(cat, "Lu")) 
  ...
  else if ((cp >= 0x80 && !strcmp(cat, "Zs")) || (cp < 0x80 && isspace(cp)))
    typ = 'w'; /* whitespace */
    ...
    typ = 'w'; /* additional whitespace */
  else if (!strcmp(cat, "Mn") || !strcmp(cat, "Nl") || !strcmp(cat, "No") || !strcmp(cat, "Pd")
        || !strcmp(cat, "Pc") || !strcmp(cat, "Po") || (cat[0] == 'S') || !strcmp(cat, "Co")
        || cp == 0x200C || cp == 0x200D)
    typ = 'i'; /* extra unicode id initial */
  else if (!strcmp(cat, "Nd") || !strcmp(cat, "Mc") || !strcmp(cat, "Me"))
    typ = 's'; /* extra unicode id subsequent */
  else
    typ = '?';
...
void emit_props(const char *out, int n)
{
  ...
  fputs("static const uint32_t uprops[] = {\n ", f);
  for (i = 0; i < n; ++i) {
