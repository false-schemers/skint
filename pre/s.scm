...
(define (%read port simple? ci?)
  ...
  ; parts of the original reader replaced by the %read-ahead builtin
  ...
  #;(define (%read-ahead-number-or-symbol fold? c p)
    (let loop ([c (peek-char p)] [l (list c)])
      (cond [(or (eof-object? c) (char-delimiter? c))
             (let* ([l (reverse! l)] [c (car l)] [s (list->string l)])
               (if (or (char-numeric? c) 
                       (char=? c #\+) (char=? c #\-) (char=? c #\.))   
                   (cond [(string=? s ".") #\.]
                         [(suspect-number-or-symbol-peculiar? #f c l s)
                          (if fold? 
                              (string->symbol (string-foldcase s))
                              (string->symbol s))]
                         [(or (string->number s) #\n)]
            ...
            [(char-symbolic? c)
             (read-char p) 
             (loop (peek-char p) (cons c l))]
            [else #\z])))

  #;(define (%read-ahead fold? p)
    (let ([c (read-char p)])
      (cond [(eof-object? c) c]
            [(char-whitespace? c) (%read-ahead fold? p)]
            [(string-position c "()[]'`,\"|#\\{}") c]
            [(char-symbolic? c) (%read-ahead-number-or-symbol fold? c p)]
            [(char=? c #\;)
  ...
  (define (sub-read p)
    ...
    (let ([c (%read-ahead fold-case? p)])
      (cond [(symbol? c) c] [(number? c) c]
            ...
            [(eqv? c #\]) close-bracket]
            [(eqv? c #\.) dot]
            [(eqv? c #\') (list 'quote (sub-read-carefully p))]
            [(eqv? c #\`) (list 'quasiquote (sub-read-carefully p))]
            [(eqv? c #\n) (r-error p "unsupported number syntax (implementation restriction)")]
            [(eqv? c #\d) (r-error p "invalid delimiter")]
            [(eqv? c #\y) (r-error p "unsupported symbol syntax")]
            [(eqv? c #\z) (r-error p "invalid token")]
            ...
            [(char=? c #\#)
             (let ([c (peek-char p)])
               (cond [(eof-object? c) (r-error p "end of file after #")]
                     ...
                     ; handled by the %read-ahead builtin
