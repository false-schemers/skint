    ...
    (let ((runner (test-runner-null)) (num-expected 0) (num-unexpected 0))
      ...
      (test-runner-on-final! runner
          (lambda (runner)
            ...
            (exit num-unexpected)))
      runner)))
(test-runner-factory test-runner-skint)
...
;[esl] originally, choke hard-failed, so can't be really catched by Skint's guard:
;(define (choke) (vector-ref '#(1 2) 3))
;this version soft-fails, so catching is possible:
(define (choke) (error "bailing out!"))
...
(test-equal
 ...
 (triv-runner
  (lambda ()
    ;; PASS
    (test-error (choke)))))  ;[esl] was (vector-ref '#(1 2) 9)
...
(test-equal
 ...
 (triv-runner
  (lambda ()
    ;; PASS
    (test-error "a" #t (choke)))))  ;[esl] was (vector-ref '#(1 2) 9)
...
(test-assert "8.3.1. default factory"
             (eq? (test-runner-factory) test-runner-skint))  ;[esl*] was test-runner-simple
