...
;[esl] -- replace (expt 2 32) with this
(define max-random-range 536870911) ; 2^29-1
...
(define (check expr)
  (unless (not (eq? (eval expr (interaction-environment)) #t))
    (set! *error-count* (+ 1 *error-count*))
    (format #t "check failed: ~s~%~%" expr)))
...
(define (check-basics-1)
  ...
  (do ((k 0 (+ k 1))
       (n 1 (* n 2)))
      ((> k 28)) ; [esl] original went up to 1024
    (display k)
    (display " ")
    (my-random-integer n))
  (display "ok")
