...
char *s_code[] = {
  ...
  "(i22),.3,.(i20),.(i24),.(i29),.(i24),.(i16),.(i27),.(i31),.(i25),.(i15"
  "),.(i28),.(i16),.(i16),.(i41),&(i17){%1.0R0,.0R8?{.0]2}.0C1?{.1,:9^[21"
  "}'(c(),.1C=?{t,:8^,.3,.3,:7^[24}'(c)),.1C=?{:8^]2}'(c[),.1C=?{t,:(i16)"
  "^,.3,.3,:7^[24}'(c]),.1C=?{:(i16)^]2}'(c'),.1C=?{${.3,:3^[01},'(y5:quo"
  "te),l2]2}'(c`),.1C=?{${.3,:3^[01},'(y10:quasiquote),l2]2}${.2,:(i15)^["
  "01}?{.1,.1,:(i10)^[22}'(c;),.1C=?{${.3R0,,#0.5,.1,&2{%1.0R8,.0?{.0]2}'"
  "(c%0a),.2C=,.0?{.0]3}:1R0,:0^[31}.!0.0^_1[01}.1,:9^[21}'(c,),.1C=?{.1R"
  "1,.0R8?{.2,'(y5:port:),'(s19:end of file after ,),@(y10:read-error)[33"
  "}'(c@),.1C=?{.2R0${.4,:3^[01},'(y16:unquote-splicing),l2]3}${.4,:3^[01"
  "},'(y7:unquote),l2]3}'(c%22),.1C=?{n,,#0.3,:(i14),.2,&3{%1:2R0,.0R8?{:"
  "2,'(y5:port:),'(s27:end of file within a string),@(y10:read-error)[23}"
  "'(c%5c),.1C=?{${'(y6:string),:2,:1^[02},.0?{.2,.1c}{.2},:0^[31}'(c%22)"
  ",.1C=?{.1A9X3]2}.1,.1c,:0^[21}.!0.0^_1[21}'(c|),.1C=?{n,,#0.3,:(i14),."
  "2,&3{%1:2R0,.0R8?{:2,'(y5:port:),'(s29:end of file within a |symbol|),"
  "@(y10:read-error)[23}'(c%5c),.1C=?{${'(y6:symbol),:2,:1^[02},.0?{.2,.1"
  "c}{.2},:0^[31}'(c|),.1C=?{.1A9X3X5]2}.1,.1c,:0^[21}.!0.0^_1[21}'(c#),."
  "1C=?{.1R1,.0R8?{.2,'(y5:port:),'(s19:end of file after #),@(y10:read-e"
  "rror)[33}'(c!),.1C=?{.2R0${.4,:(i12)^[01},.0,'(l2:y9:fold-case;y12:no-"
  "fold-case;),.1A1?{'(y9:fold-case),.2q:!(i13):(i13)^,.5P79.4,:9^[51}.1Y"
  "0?{.1Y6]5}.4,'(y5:port:),.3,'(s24:unexpected name after #!),@(y10:read"
  "-error)[54}'(ct),.1Ci=,.0?{.0}{'(cf),.2Ci=,.0?{.0}{'(cs),.3Ci=,.0?{.0}"
  "{'(cu),.4Ci=}_1}_1}_1?{${.4,:3^[01},.0,'(l2:y1:t;y4:true;),.1A1?{t]5}'"
  "(l2:y1:f;y5:false;),.1A1?{f]5}'(y2:u8),.1v?{'0,${.4,.8,:(i11)^[02}E2]5"
  "}'(y2:s8),.1v?{'1,${.4,.8,:(i11)^[02}E2]5}'(y3:u16),.1v?{'2,${.4,.8,:("
  "i11)^[02}E2]5}'(y3:s16),.1v?{'3,${.4,.8,:(i11)^[02}E2]5}'(y3:f32),.1v?"
  "{'(i10),${.4,.8,:(i11)^[02}E2]5}'(y3:f64),.1v?{'(i11),${.4,.8,:(i11)^["
  "02}E2]5}.4,'(y5:port:),.3,'(s23:unexpected name after #),@(y10:read-er"
  "ror)[54}'(cb),.1Ci=,.0?{.0}{'(co),.2Ci=,.0?{.0}{'(cd),.3Ci=,.0?{.0}{'("
  "cx),.4Ci=,.0?{.0}{'(ci),.5Ci=,.0?{.0}{'(ce),.6Ci=}_1}_1}_1}_1}_1?{.2,'"
  "(c#),:(i10)^[32}'(c&),.1C=?{.2R0${.4,:3^[01}b]3}'(c;),.1C=?{.2R0${.4,:"
  "3^[01}.2,:9^[31}'(c|),.1C=?{.2R0${,#0.5,.1,&2{%0:1R0,.0R8?{:1,'(y5:por"
  "t:),'(s25:end of file in #| comment),@(y10:read-error)[13}'(c|),.1C=?{"
  ":1R1,.0R8?{:1,'(y5:port:),'(s25:end of file in #| comment),@(y10:read-"
  "error)[23}'(c#),.1C=?{:1R0]2}:0^[20}'(c#),.1C=?{:1R1,.0R8?{:1,'(y5:por"
  "t:),'(s25:end of file in #| comment),@(y10:read-error)[23}'(c|),.1C=?{"
  ":1R0${:0^[00}:0^[20}:0^[20}:0^[10}.!0.0^_1[00}.2,:9^[31}'(c(),.1C=?{.2"
  "R0${f,:8^,.6,.5,:7^[04}X1]3}'(c%5c),.1C=?{.2R0.2R1,.0R8?{.3,'(y5:port:"
  "),'(s20:end of file after #%5c),@(y10:read-error)[43}.0,'(cx)C=?{.3R0$"
  "{.5R1,:5^[01}?{.0]4}f,.4,:6^[42}.0C4?{${.5,:3^[01},'1,.1X4S3=?{.1]5}.0"
  ",'(y4:null),.1v?{'0X9]6}'(y5:space),.1v?{'(c )]6}'(y5:alarm),.1v?{'(c%"
  "07)]6}'(y9:backspace),.1v?{'(c%08)]6}'(y6:delete),.1v?{'(i127)X9]6}'(y"
  "6:escape),.1v?{'(i27)X9]6}'(y3:tab),.1v?{'(c%09)]6}'(l2:y7:newline;y8:"
  "linefeed;),.1A1?{'(c%0a)]6}'(y4:vtab),.1v?{'(c%0b)]6}'(y4:page),.1v?{'"
  "(c%0c)]6}'(y6:return),.1v?{'(c%0d)]6}.5,'(y5:port:),.3,'(s15:unknown #"
  "%5c name),@(y10:read-error)[64}.3R0.0]4}.0C5?{:0?{${.4,'(y5:port:),'(s"
  "44:#N=/#N# notation is not allowed in this mode),@(y10:read-error)[03}"
  "}n,,#0.4,.1,:4,:3,:2,:1,&6{%1:5R0,.0R8?{:5,'(y5:port:),'(s32:end of fi"
  "le within a #N notation),@(y10:read-error)[23}.0C5?{.1,.1c,:4^[21}'(c#"
  "),.1C=?{.1A9X3,'(i10),.1E9,.0I0?{:0^,.1A3}{f},.0?{.0d]5}'(s22:unknown "
  "#n# reference:),'(y5:port:),.4,@(y10:read-error)[53}'(c=),.1C=?{.1A9X3"
  ",'(i10),.1E9,.0I0~?{${'(s22:invalid #n= reference:),'(y5:port:),.5,@(y"
  "10:read-error)[03}}{:0^,.1A3?{${'(s18:duplicate #n= tag:),'(y5:port:),"
  ".4,@(y10:read-error)[03}}{f}}fb,:0^,${.3,:1^[01},.3cc:!0${:5,:2^[01},$"
  "{.2,:3^[01}?{'(s31:#n= has another label as target),'(y5:port:),.5,@(y"
  "10:read-error)[63}.0,.2sz.0]6}:5,'(y5:port:),'(s34:invalid terminator "
  "for #N notation),@(y10:read-error)[23}.!0.0^_1[31}.2,'(y5:port:),.2,'("
  "s16:unknown # syntax),@(y10:read-error)[34}.1,'(y5:port:),.2,'(s22:ill"
  "egal character read),@(y10:read-error)[24}.!(i17).(i17),.(i12),.(i17),"
  ".(i13),&4{%4${.3,:3^[01},:0^,.1q?{.2,'(y5:port:),'(s42:missing car -- "
  "( immediately followed by .),@(y10:read-error)[53}.0,,#0.0,.5,:3,:2,.("
  "i10),:1,.(i11),:0,&8{%1.0R8?{:6,'(y5:port:),'(s41:eof inside list -- u"
  "nbalanced parentheses),@(y10:read-error)[13}:1,.1q?{n]1}:0^,.1q?{:3?{$"
  "{:6,:2^[01},${:6,:5^[01},:1,.1q?{.1]3}:6,'(y5:port:),.2,'(s31:randomne"
  "ss after form after dot),@(y10:read-error)[34}:6,'(y5:port:),'(s13:dot"
  " in #(...)),@(y10:read-error)[13}${.2,:4^[01}?{:6,'(y5:port:),.2d,'(s2"
  "0:error inside list --),@(y10:read-error)[14}${${:6,:5^[01},:7^[01},.1"
  "c]1}.!0.0^_1[51}.!(i18).(i11),.9,.(i19),&3{%2'(c(),.1R0q~?{${.2,'(y5:p"
  "ort:),${.7,'(s23:invalid ~avector syntax),@(y6:format)[02},@(y10:read-"
  "error)[03}}${.2,:0^[01},,#0.2,.4,:0,.3,:2,:1,&6{%1.0R8?{:5,'(y5:port:)"
  ",${:4,'(s19:eof inside ~avector),@(y6:format)[02},@(y10:read-error)[13"
  "}:0^,.1q?{n]1}${.2,:1^[01}?{:5,'(y5:port:),.2d,${:4,'(s24:error inside"
  " ~avector --),@(y6:format)[02},@(y10:read-error)[14}'(y2:u8),:4q?{.0I0"
  "?{'(i255),.1,,'0I>!;I>!}{f}}{f},.0?{.0}{'(y2:s8),:4q?{.1I0?{'(i127),.2"
  ",,'(i-128)I>!;I>!}{f}}{f},.0?{.0}{'(y3:u16),:4q?{.2I0?{'(i65535),.3,,'"
  "0I>!;I>!}{f}}{f},.0?{.0}{'(y3:s16),:4q?{.3I0?{'(i32767),.4,,'(i-32768)"
  "I>!;I>!}{f}}{f},.0?{.0}{'(y3:f32),:4q?{.4J0}{f},.0?{.0}{'(y3:f64),:4q?"
  "{.5J0}{f}}_1}_1}_1}_1}_1?{${${:5,:3^[01},:2^[01},.1c]1}:5,'(y5:port:),"
  ".2,${:4,:4,'(s29:invalid ~a inside ~avector --),@(y6:format)[03},@(y10"
  ":read-error)[14}.!0.0^_1[21}.!(i19).(i21),&1{%2.0R0,.0R8?{${.3,'(y5:po"
  "rt:),.6,'(s20:end of file within a),@(y10:read-error)[04}}'(c%5c),.1C="
  ",.0?{.0}{'(c%22),.2C=,.0?{.0}{'(c|),.3C=}_1}_1?{.0]3}'(ca),.1C=?{'(c%0"
  "7)]3}'(cb),.1C=?{'(c%08)]3}'(ct),.1C=?{'(c%09)]3}'(cn),.1C=?{'(c%0a)]3"
  "}'(cv),.1C=?{'(c%0b)]3}'(cf),.1C=?{'(c%0c)]3}'(cr),.1C=?{'(c%0d)]3}'(c"
  "x),.1C=?{t,.2,:0^[32}'(y6:string),.3q?{.0C1}{f}?{.1R1,'(c%0a),.2C=,,#0"
  ".0,.5,&2{%2.1R8,.0?{.0}{.2C1~}_1?{.0?{f]2}:0,'(y5:port:),'(s32:no newl"
  "ine in line ending escape),@(y10:read-error)[23}.0?{'(c%0a),.2C=}{f}?{"
  "f]2}:0R0:0R1,.1,.0?{.0}{'(c%0a),.4C=}_1,:1^[22}.!0.0^_1[32}.1,'(y5:por"
  "t:),.2,'(y1::),.6,'(s22:invalid char escape in),@(y10:read-error)[36}."
  "!(i20).(i14),.(i14),&2{%2,#0.1,&1{%1.0u?{:0,'(y5:port:),'(s31:%5cx esc"
  "ape sequence is too short),@(y10:read-error)[13}'(i16),.1A9X3X7X9]1}.!"
  "0'0,n,.3R1,,#0.0,.6,:0,.7,.(i10),:1,&6{%3.0R8?{:1?{:4,'(y5:port:),'(s2"
  "7:end of file within a string),@(y10:read-error)[33}.1,:2^[31}:1?{'(c;"
  "),.1C=}{f}?{:4R0.1,:2^[31}:1~?{${.2,:0^[01}}{f}?{.1,:2^[31}${.2,:3^[01"
  "}~?{:4,'(y5:port:),.2,'(s37:unexpected char in %5cx escape sequence),@"
  "(y10:read-error)[34}'2,.3>?{:4,'(y5:port:),'(s30:%5cx escape sequence "
  "is too long),@(y10:read-error)[33}:4R0'1,.3+,.2,.2c,:4R1,:5^[33}.!0.0^"
  "_1[33}.!(i21)&0{%4.0,.0?{.0}{.2C5}_1?{f]4}'(s2:+i),.4Si=,.0?{.0}{'(s2:"
  "-i),.5Si=}_1?{f]4}'(s6:+nan.0),.4Si=,.0?{.0}{'(s6:-nan.0),.5Si=}_1?{f]"
  "4}'(s6:+inf.0),.4Si=,.0?{.0}{'(s6:-inf.0),.5Si=}_1?{f]4}'(c+),.2C=,.0?"
  "{.0}{'(c-),.3C=}_1?{.2du?{t]4}'(c.),.3daC=?{.2ddp?{.2ddaC5~]4}f]4}.2da"
  "C5~]4}'(c.),.2C=?{.2dp?{.2daC5~]4}f]4}f]4}.!(i22).(i14),.(i23),.(i12),"
  ".3,.(i16),&5{%2'(c#),.1C=,.1,l1,.3R1,,#0.5,.1,:0,:1,:2,:3,:4,&7{%3.0R8"
  ",.0?{.0}{${.3,:0^[01}}_1?{.1A9,.0a,.1X3,.5,.0?{.0}{.2C5,.0?{.0}{'(c+),"
  ".4C=,.0?{.0}{'(c-),.5C=,.0?{.0}{'(c.),.6C=}_1}_1}_1}_1?{'(s1:.),.1S=?{"
  ":2^]6}${.2,.5,.5,.(i10),:1^[04}?{:3^?{.0SfX5]6}.0X5]6}'(i10),.1E9,.0?{"
  ".0]7}:6,'(y5:port:),.3,'(s54:unsupported number syntax (implementation"
  " restriction)),@(y10:read-error)[74}:3^?{.0SfX5]6}.0X5]6}'(c#),.1C=?{:"
  "6R0t,.2,.2c,:6R1,:5^[33}${.2,:4^[01}?{:6R0.2,.2,.2c,:6R1,:5^[33}:6,'(y"
  "5:port:),.2,'(s29:unexpected number/symbol char),@(y10:read-error)[34}"
  ".!0.0^_1[23}.!(i23)${.(i26),.(i20)^[01},${.2,.(i15)^[01}~?{.2^u?{.0](i"
  "28)}.0,.8^[(i28)1}.(i25),'(y5:port:),.2d,'(s17:unexpected token:),@(y1"
  "0:read-error)[(i28)4",
