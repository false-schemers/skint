...
char *s_code[] = {
  ...
  "6),.1v?{'3,${.4,.8,:(i10)^[02}E2]5}'(y3:u32),.1v?{'4,${.4,.8,:(i10)^[0"
  "2}E2]5}'(y3:s32),.1v?{'5,${.4,.8,:(i10)^[02}E2]5}'(y3:u64),.1v?{'6,${."
  "4,.8,:(i10)^[02}E2]5}'(y3:s64),.1v?{'7,${.4,.8,:(i10)^[02}E2]5}'(y3:f3"
  "2),.1v?{'(i10),${.4,.8,:(i10)^[02}E2]5}'(y3:f64),.1v?{'(i11),${.4,.8,:"
  "(i10)^[02}E2]5}.1,'(s23:unexpected name after #),.6,@(y10:read-error)["
  "53}'(c&),.1C=?{.2R0${.4,:3^[01}b]3}'(c;),.1C=?{.2R0${.4,:3^[01}.2,:9^["
  "31}'(c|),.1C=?{.2R0${,#0.5,.1,&2{%0:1R0,.0R8?{'(s25:end of file in #| "
  "comment),:1,@(y10:read-error)[12}'(c|),.1C=?{:1R1,.0R8?{'(s25:end of f"
  "ile in #| comment),:1,@(y10:read-error)[22}'(c#),.1C=?{:1R0]2}:0^[20}'"
  "(c#),.1C=?{:1R1,.0R8?{'(s25:end of file in #| comment),:1,@(y10:read-e"
  "rror)[22}'(c|),.1C=?{:1R0${:0^[00}:0^[20}:0^[20}:0^[10}.!0.0^_1[00}.2,"
  ":9^[31}'(c(),.1C=?{.2R0${f,:8^,.6,.5,:7^[04}X1]3}'(c%5c),.1C=?{.2R0.2R"
  "1,.0R8?{'(s20:end of file after #%5c),.4,@(y10:read-error)[42}.0,'(cx)"
  "C=?{.3R0${.5R1,:5^[01}?{.0]4}f,.4,:6^[42}.0C4?{${.5,:3^[01},'1,.1X4S3="
  "?{.1]5}.0,'(y4:null),.1v?{'0X9]6}'(y5:space),.1v?{'(c )]6}'(y5:alarm),"
  ".1v?{'(c%07)]6}'(y9:backspace),.1v?{'(c%08)]6}'(y6:delete),.1v?{'(i127"
  ")X9]6}'(y6:escape),.1v?{'(i27)X9]6}'(y3:tab),.1v?{'(c%09)]6}'(l2:y7:ne"
  "wline;y8:linefeed;),.1A1?{'(c%0a)]6}'(y4:vtab),.1v?{'(c%0b)]6}'(y4:pag"
  "e),.1v?{'(c%0c)]6}'(y6:return),.1v?{'(c%0d)]6}.1,'(s15:unknown #%5c na"
  "me),.7,@(y10:read-error)[63}.3R0.0]4}.0C5?{:0?{${'(s44:#N=/#N# notatio"
  "n is not allowed in this mode),.5,@(y10:read-error)[02}}n,,#0.4,.1,:4,"
  ":3,:2,:1,&6{%1:5R0,.0R8?{'(s32:end of file within a #N notation),:5,@("
  "y10:read-error)[22}.0C5?{.1,.1c,:4^[21}'(c#),.1C=?{.1A9X3,'(i10),.1E9,"
  ".0I0?{:0^,.1A3}{f},.0?{.0d]5}.2,'(s22:unknown #n# reference:),:5,@(y10"
  ":read-error)[53}'(c=),.1C=?{.1A9X3,'(i10),.1E9,.0I0~?{${.3,'(s22:inval"
  "id #n= reference:),:5,@(y10:read-error)[03}}{:0^,.1A3?{${.2,'(s18:dupl"
  "icate #n= tag:),:5,@(y10:read-error)[03}}{f}}fb,:0^,${.3,:1^[01},.3cc:"
  "!0${:5,:2^[01},${.2,:3^[01}?{.3,'(s31:#n= has another label as target)"
  ",:5,@(y10:read-error)[63}.0,.2sz.0]6}'(s34:invalid terminator for #N n"
  "otation),:5,@(y10:read-error)[22}.!0.0^_1[31}.0,'(s16:unknown # syntax"
  "),.4,@(y10:read-error)[33}.0,'(s22:illegal character read),.3,@(y10:re"
  "ad-error)[23}.!(i18).(i18),.(i14),.(i18),.(i15),&4{%4${.3,:3^[01},:0^,"
  ".1q?{'(s42:missing car -- ( immediately followed by .),.3,@(y10:read-e"
  "rror)[52}.0,,#0.0,.5,:3,:2,.(i10),:1,.(i11),:0,&8{%1.0R8?{'(s41:eof in"
  "side list -- unbalanced parentheses),:6,@(y10:read-error)[12}:1,.1q?{n"
  "]1}:0^,.1q?{:3?{${:6,:2^[01},${:6,:5^[01},:1,.1q?{.1]3}.0,'(s31:random"
  "ness after form after dot),:6,@(y10:read-error)[33}'(s13:dot in #(...)"
  "),:6,@(y10:read-error)[12}${.2,:4^[01}?{.0d,'(s20:error inside list --"
  "),:6,@(y10:read-error)[13}${${:6,:5^[01},:7^[01},.1c]1}.!0.0^_1[51}.!("
  "i19).(i13),.(i11),.(i20),&3{%2'(l4:y3:u32;y3:s32;y3:u64;y3:s64;),.2A0?"
  "{@(y9:opt-tower)~}{f}?{${@(y4:name),'(s33:unsupported numerical vector"
  " type),.4,@(y10:read-error)[03}}'(c(),.1R0q~?{${${.5,'(s23:invalid ~av"
  "ector syntax),@(y6:format)[02},.3,@(y10:read-error)[02}}${.2,:0^[01},,"
  "#0.3,.3,:0,.3,:2,:1,&6{%1.0R8?{${:5,'(s19:eof inside ~avector),@(y6:fo"
  "rmat)[02},:4,@(y10:read-error)[12}:0^,.1q?{n]1}${.2,:1^[01}?{.0d,${:5,"
  "'(s24:error inside ~avector --),@(y6:format)[02},:4,@(y10:read-error)["
  "13}'(y2:u8),:5q?{.0I0?{'(i255),.1,,'0I>!;I>!}{f}}{f},.0?{.0}{'(y2:s8),"
  ":5q?{.1I0?{'(i127),.2,,'(i-128)I>!;I>!}{f}}{f},.0?{.0}{'(y3:u16),:5q?{"
  ".2I0?{'(i65535),.3,,'0I>!;I>!}{f}}{f},.0?{.0}{'(y3:s16),:5q?{.3I0?{'(i"
  "32767),.4,,'(i-32768)I>!;I>!}{f}}{f},.0?{.0}{'(y3:u32),:5q?{.4Nw!?{'0,"
  ".5<!?{'(i32),.5G8I>!}{f}}{f}}{f},.0?{.0}{'(y3:s32),:5q?{.5Nw!?{'(i32),"
  ".6G8I<}{f}}{f},.0?{.0}{'(y3:u64),:5q?{.6Nw!?{'0,.7<!?{'(i64),.7G8I>!}{"
  "f}}{f}}{f},.0?{.0}{'(y3:s64),:5q?{.7Nw!?{'(i64),.8G8I<}{f}}{f},.0?{.0}"
  "{'(y3:f32),:5q?{.8J0}{f},.0?{.0}{'(y3:f64),:5q?{.9J0}{f}}_1}_1}_1}_1}_"
