...
char *s_code[] = {
  ...
  "=,.0?{.0}{'(cf),.2Ci=,.0?{.0}{'(cs),.3Ci=,.0?{.0}{'(cu),.4Ci=,.0?{.0}{"
  "'(cc),.5Ci=}_1}_1}_1}_1?{${.4,:3^[01},${.2,@(y15:symbol-downcase)[01},"
  ".0,'(l2:y1:t;y4:true;),.1A1?{t]6}'(l2:y1:f;y5:false;),.1A1?{f]6}'(y2:u"
  "8),.1v?{'0,${.4,.9,:(i10)^[02}E2]6}'(y2:s8),.1v?{'1,${.4,.9,:(i10)^[02"
  "}E2]6}'(y3:u16),.1v?{'2,${.4,.9,:(i10)^[02}E2]6}'(y3:s16),.1v?{'3,${.4"
  ",.9,:(i10)^[02}E2]6}'(y3:u32),.1v?{'4,${.4,.9,:(i10)^[02}E2]6}'(y3:s32"
  "),.1v?{'5,${.4,.9,:(i10)^[02}E2]6}'(y3:u64),.1v?{'6,${.4,.9,:(i10)^[02"
  "}E2]6}'(y3:s64),.1v?{'7,${.4,.9,:(i10)^[02}E2]6}'(y3:f32),.1v?{'(i10),"
  "${.4,.9,:(i10)^[02}E2]6}'(y3:f64),.1v?{'(i11),${.4,.9,:(i10)^[02}E2]6}"
  "'(y3:c64),.1v?{'(i14),${.4,.9,:(i10)^[02}E2]6}'(y4:c128),.1v?{'(i15),$"
  "{.4,.9,:(i10)^[02}E2]6}.2,'(s23:unexpected name after #),.7,@(y10:read"
  "-error)[63}'(c&),.1C=?{.2R0${.4,:3^[01}b]3}'(c;),.1C=?{.2R0${.4,:3^[01"
  ...
  "@(y10:read-error)[23}.!(i18).(i18),.(i14),.(i18),.(i15),&4{%4${.3,:3^["
  ...
  "1[51}.!(i19).(i13),.(i11),.(i20),&3{%2'(l6:y3:u32;y3:s32;y3:u64;y3:s64"
  ";y3:c64;y4:c128;),.2A0?{@(y9:opt-tower)~}{f}?{${@(y4:name),'(s33:unsup"
  "ported numerical vector type),.4,@(y10:read-error)[03}}'(c(),.1R0q~?{$"
  "{${.5,'(s23:invalid ~avector syntax),@(y6:format)[02},.3,@(y10:read-er"
  "ror)[02}}${.2,:0^[01},,#0.3,.3,:0,.3,:2,:1,&6{%1.0R8?{${:5,'(s19:eof i"
  "nside ~avector),@(y6:format)[02},:4,@(y10:read-error)[12}:0^,.1q?{n]1}"
  "${.2,:1^[01}?{.0d,${:5,'(s24:error inside ~avector --),@(y6:format)[02"
  "},:4,@(y10:read-error)[13}'(y2:u8),:5q?{.0I0?{'(i255),.1,,'0I>!;I>!}{f"
  "}}{f},.0?{.0}{'(y2:s8),:5q?{.1I0?{'(i127),.2,,'(i-128)I>!;I>!}{f}}{f},"
  ".0?{.0}{'(y3:u16),:5q?{.2I0?{'(i65535),.3,,'0I>!;I>!}{f}}{f},.0?{.0}{'"
  "(y3:s16),:5q?{.3I0?{'(i32767),.4,,'(i-32768)I>!;I>!}{f}}{f},.0?{.0}{'("
  "y3:u32),:5q?{.4Nw!?{'0,.5<!?{'(i32),.5G8I>!}{f}}{f}}{f},.0?{.0}{'(y3:s"
  "32),:5q?{.5Nw!?{'(i32),.6G8I<}{f}}{f},.0?{.0}{'(y3:u64),:5q?{.6Nw!?{'0"
  ...
  ".(i11)Tj}{f}}_1}_1}_1}_1}_1}_1}_1}_1}_1}_1}_1?{${${:4,:3^[01},:2^[01},"
  ".1c]1}.0,${:5,:5,'(s29:invalid ~a inside ~avector --),@(y6:format)[03}"
  ",:4,@(y10:read-error)[13}.!0.0^_1[21}.!(i20).(i22),&1{%2.0R0,.0R8?{${."
  "4,'(s20:end of file within a),.5,@(y10:read-error)[03}}'(c%5c),.1C=,.0"
  "?{.0}{'(c%22),.2C=,.0?{.0}{'(c|),.3C=}_1}_1?{.0]3}'(ca),.1C=?{'(c%07)]"
  "3}'(cb),.1C=?{'(c%08)]3}'(ct),.1C=?{'(c%09)]3}'(cn),.1C=?{'(c%0a)]3}'("
  "cv),.1C=?{'(c%0b)]3}'(cf),.1C=?{'(c%0c)]3}'(cr),.1C=?{'(c%0d)]3}'(cx),"
  ".1C=?{t,.2,:0^[32}'(y6:string),.3q?{.0C1}{f}?{.1R1,'(c%0a),.2C=,,#0.0,"
  ".5,&2{%2.1R8,.0?{.0}{.2C1~}_1?{.0?{f]2}'(s32:no newline in line ending"
  " escape),:0,@(y10:read-error)[22}.0?{'(c%0a),.2C=}{f}?{f]2}:0R0:0R1,.1"
  ",.0?{.0}{'(c%0a),.4C=}_1,:1^[22}.!0.0^_1[32}.0,'(y1::),.4,'(s22:invali"
  "d char escape in),.5,@(y10:read-error)[35}.!(i21).7,.(i16),.(i16),.(i1"
  "1),&4{%2,#0:3,.2,&2{%1.0u?{'(s31:%5cx escape sequence is too short),:0"
  ",@(y10:read-error)[12}'(i16),.1A9X3X7,:1^,.1>?{'(s27:%5cx escape seque"
  "nce overflow),:0,@(y10:read-error)[22}.0X9]2}.!0'0,n,.3R1,,#0.0,.6,:0,"
  ":1,.8,.(i11),:2,&7{%3.0R8?{:1?{'(s27:end of file within a string),:5,@"
  "(y10:read-error)[32}.1,:2^[31}:1?{'(c;),.1C=}{f}?{:5R0.1,:2^[31}:1~?{$"
  "{.2,:0^[01}}{f}?{.1,:2^[31}${.2,:3^[01}~?{.0,'(s37:unexpected char in "
  "%5cx escape sequence),:5,@(y10:read-error)[33}:4^,.3>?{'(s30:%5cx esca"
  "pe sequence is too long),:5,@(y10:read-error)[32}:5R0'1,.3+,.2,.2c,:5R"
  "1,:6^[33}.!0.0^_1[33}.!(i22)${.(i25),.(i21)^[01},${.2,.(i17)^[01}~?{.2"
  "^u?{.0](i27)}.0,.8^[(i27)1}.0d,'(s17:unexpected token:),.(i26),@(y10:r"
  "ead-error)[(i27)3",
