...
char *s_code[] = {
  ...
  "%1${.2,:(i15)^,@(y11:%25read-ahead)[02},.0Y0?{.0]2}.0N0?{.0]2}.0R8?{.0"
  "]2}'(c(),.1v?{t,:8^,.3,.3,:7^[24}'(c)),.1v?{:8^]2}'(c[),.1v?{t,:(i14)^"
  ",.3,.3,:7^[24}'(c]),.1v?{:(i14)^]2}'(c.),.1v?{:(i13)^]2}'(c'),.1v?{${."
  "3,:3^[01},'(y5:quote),l2]2}'(c`),.1v?{${.3,:3^[01},'(y10:quasiquote),l"
  "2]2}'(cn),.1v?{'(s54:unsupported number syntax (implementation restric"
  "tion)),.2,@(y10:read-error)[22}'(cd),.1v?{'(s17:invalid delimiter),.2,"
  "@(y10:read-error)[22}'(cy),.1v?{'(s25:unsupported symbol syntax),.2,@("
  "y10:read-error)[22}'(cz),.1v?{'(s13:invalid token),.2,@(y10:read-error"
  ")[22}'(c,),.1C=?{.1R1,.0R8?{'(s19:end of file after ,),.3,@(y10:read-e"
  "rror)[32}'(c@),.1C=?{.2R0${.4,:3^[01},'(y16:unquote-splicing),l2]3}${."
  "4,:3^[01},'(y7:unquote),l2]3}'(c%22),.1C=?{n,,#0.3,:(i12),.2,&3{%1:2R0"
  ",.0R8?{'(s27:end of file within a string),:2,@(y10:read-error)[22}'(c%"
  "5c),.1C=?{${'(y6:string),:2,:1^[02},.0?{.2,.1c}{.2},:0^[31}'(c%22),.1C"
  "=?{.1A9X3]2}.1,.1c,:0^[21}.!0.0^_1[21}'(c|),.1C=?{n,,#0.3,:(i12),.2,&3"
  "{%1:2R0,.0R8?{'(s29:end of file within a |symbol|),:2,@(y10:read-error"
  ")[22}'(c%5c),.1C=?{${'(y6:symbol),:2,:1^[02},.0?{.2,.1c}{.2},:0^[31}'("
  "c|),.1C=?{.1A9X3X5]2}.1,.1c,:0^[21}.!0.0^_1[21}'(c#),.1C=?{.1R1,.0R8?{"
  "'(s19:end of file after #),.3,@(y10:read-error)[32}'(c!),.1C=?{.2R0${."
  "4,:(i11)^[01},.0,'(l2:y9:fold-case;y12:no-fold-case;),.1A1?{'(y9:fold-"
  "case),.2q:!(i15):(i15)^,.5P79.4,:9^[51}.1Y0?{.1Y6]5}.1,'(s24:unexpecte"
  "d name after #!),.6,@(y10:read-error)[53}'(ct),.1Ci=,.0?{.0}{'(cf),.2C"
  "i=,.0?{.0}{'(cs),.3Ci=,.0?{.0}{'(cu),.4Ci=}_1}_1}_1?{${.4,:3^[01},.0,'"
  "(l2:y1:t;y4:true;),.1A1?{t]5}'(l2:y1:f;y5:false;),.1A1?{f]5}'(y2:u8),."
  "1v?{'0,${.4,.8,:(i10)^[02}E2]5}'(y2:s8),.1v?{'1,${.4,.8,:(i10)^[02}E2]"
  "5}'(y3:u16),.1v?{'2,${.4,.8,:(i10)^[02}E2]5}'(y3:s16),.1v?{'3,${.4,.8,"
  ":(i10)^[02}E2]5}'(y3:f32),.1v?{'(i10),${.4,.8,:(i10)^[02}E2]5}'(y3:f64"
  "),.1v?{'(i11),${.4,.8,:(i10)^[02}E2]5}.1,'(s23:unexpected name after #"
  "),.6,@(y10:read-error)[53}'(c&),.1C=?{.2R0${.4,:3^[01}b]3}'(c;),.1C=?{"
  ".2R0${.4,:3^[01}.2,:9^[31}'(c|),.1C=?{.2R0${,#0.5,.1,&2{%0:1R0,.0R8?{'"
  ...
  "}:0^[10}.!0.0^_1[00}.2,:9^[31}'(c(),.1C=?{.2R0${f,:8^,.6,.5,:7^[04}X1]"
  "3}'(c%5c),.1C=?{.2R0.2R1,.0R8?{'(s20:end of file after #%5c),.4,@(y10:"
  "read-error)[42}.0,'(cx)C=?{.3R0${.5R1,:5^[01}?{.0]4}f,.4,:6^[42}.0C4?{"
  ...
  "ror)[02}}n,,#0.4,.1,:4,:3,:2,:1,&6{%1:5R0,.0R8?{'(s32:end of file with"
  "in a #N notation),:5,@(y10:read-error)[22}.0C5?{.1,.1c,:4^[21}'(c#),.1"
  "C=?{.1A9X3,'(i10),.1E9,.0I0?{:0^,.1A3}{f},.0?{.0d]5}.2,'(s22:unknown #"
  "n# reference:),:5,@(y10:read-error)[53}'(c=),.1C=?{.1A9X3,'(i10),.1E9,"
  ".0I0~?{${.3,'(s22:invalid #n= reference:),:5,@(y10:read-error)[03}}{:0"
  "^,.1A3?{${.2,'(s18:duplicate #n= tag:),:5,@(y10:read-error)[03}}{f}}fb"
  ",:0^,${.3,:1^[01},.3cc:!0${:5,:2^[01},${.2,:3^[01}?{.3,'(s31:#n= has a"
  "nother label as target),:5,@(y10:read-error)[63}.0,.2sz.0]6}'(s34:inva"
  "lid terminator for #N notation),:5,@(y10:read-error)[22}.!0.0^_1[31}.0"
  ",'(s16:unknown # syntax),.4,@(y10:read-error)[33}.0,'(s22:illegal char"
  "acter read),.3,@(y10:read-error)[23}.!(i16).(i16),.(i12),.(i16),.(i13)"
  ",&4{%4${.3,:3^[01},:0^,.1q?{'(s42:missing car -- ( immediately followe"
  "d by .),.3,@(y10:read-error)[52}.0,,#0.0,.5,:3,:2,.(i10),:1,.(i11),:0,"
  "&8{%1.0R8?{'(s41:eof inside list -- unbalanced parentheses),:6,@(y10:r"
  "ead-error)[12}:1,.1q?{n]1}:0^,.1q?{:3?{${:6,:2^[01},${:6,:5^[01},:1,.1"
  "q?{.1]3}.0,'(s31:randomness after form after dot),:6,@(y10:read-error)"
  "[33}'(s13:dot in #(...)),:6,@(y10:read-error)[12}${.2,:4^[01}?{.0d,'(s"
  "20:error inside list --),:6,@(y10:read-error)[13}${${:6,:5^[01},:7^[01"
  "},.1c]1}.!0.0^_1[51}.!(i17).(i11),.9,.(i18),&3{%2'(c(),.1R0q~?{${${.5,"
  "'(s23:invalid ~avector syntax),@(y6:format)[02},.3,@(y10:read-error)[0"
  "2}}${.2,:0^[01},,#0.3,.3,:0,.3,:2,:1,&6{%1.0R8?{${:5,'(s19:eof inside "
  "~avector),@(y6:format)[02},:4,@(y10:read-error)[12}:0^,.1q?{n]1}${.2,:"
  "1^[01}?{.0d,${:5,'(s24:error inside ~avector --),@(y6:format)[02},:4,@"
  "(y10:read-error)[13}'(y2:u8),:5q?{.0I0?{'(i255),.1,,'0I>!;I>!}{f}}{f},"
  ".0?{.0}{'(y2:s8),:5q?{.1I0?{'(i127),.2,,'(i-128)I>!;I>!}{f}}{f},.0?{.0"
  "}{'(y3:u16),:5q?{.2I0?{'(i65535),.3,,'0I>!;I>!}{f}}{f},.0?{.0}{'(y3:s1"
  "6),:5q?{.3I0?{'(i32767),.4,,'(i-32768)I>!;I>!}{f}}{f},.0?{.0}{'(y3:f32"
  "),:5q?{.4J0}{f},.0?{.0}{'(y3:f64),:5q?{.5J0}{f}}_1}_1}_1}_1}_1?{${${:4"
  ",:3^[01},:2^[01},.1c]1}.0,${:5,:5,'(s29:invalid ~a inside ~avector --)"
  ",@(y6:format)[03},:4,@(y10:read-error)[13}.!0.0^_1[21}.!(i18).(i20),&1"
  "{%2.0R0,.0R8?{${.4,'(s20:end of file within a),.5,@(y10:read-error)[03"
  "}}'(c%5c),.1C=,.0?{.0}{'(c%22),.2C=,.0?{.0}{'(c|),.3C=}_1}_1?{.0]3}'(c"
  "a),.1C=?{'(c%07)]3}'(cb),.1C=?{'(c%08)]3}'(ct),.1C=?{'(c%09)]3}'(cn),."
  "1C=?{'(c%0a)]3}'(cv),.1C=?{'(c%0b)]3}'(cf),.1C=?{'(c%0c)]3}'(cr),.1C=?"
  "{'(c%0d)]3}'(cx),.1C=?{t,.2,:0^[32}'(y6:string),.3q?{.0C1}{f}?{.1R1,'("
  "c%0a),.2C=,,#0.0,.5,&2{%2.1R8,.0?{.0}{.2C1~}_1?{.0?{f]2}'(s32:no newli"
  "ne in line ending escape),:0,@(y10:read-error)[22}.0?{'(c%0a),.2C=}{f}"
  "?{f]2}:0R0:0R1,.1,.0?{.0}{'(c%0a),.4C=}_1,:1^[22}.!0.0^_1[32}.0,'(y1::"
  "),.4,'(s22:invalid char escape in),.5,@(y10:read-error)[35}.!(i19).(i1"
  "3),.(i13),&2{%2,#0.1,&1{%1.0u?{'(s31:%5cx escape sequence is too short"
  "),:0,@(y10:read-error)[12}'(i16),.1A9X3X7X9]1}.!0'0,n,.3R1,,#0.0,.6,:0"
  ",.7,.(i10),:1,&6{%3.0R8?{:1?{'(s27:end of file within a string),:4,@(y"
  "10:read-error)[32}.1,:2^[31}:1?{'(c;),.1C=}{f}?{:4R0.1,:2^[31}:1~?{${."
  "2,:0^[01}}{f}?{.1,:2^[31}${.2,:3^[01}~?{.0,'(s37:unexpected char in %5"
  "cx escape sequence),:4,@(y10:read-error)[33}'2,.3>?{'(s30:%5cx escape "
  "sequence is too long),:4,@(y10:read-error)[32}:4R0'1,.3+,.2,.2c,:4R1,:"
  "5^[33}.!0.0^_1[33}.!(i20)${.(i23),.(i19)^[01},${.2,.(i15)^[01}~?{.2^u?"
  "{.0](i25)}.0,.8^[(i25)1}.0d,'(s17:unexpected token:),.(i24),@(y10:read"
  "-error)[(i25)3",
