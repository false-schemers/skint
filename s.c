...
char *s_code[] = {
  ...
  "'(cc),.5Ci=}_1}_1}_1}_1?{${.4,:3^[01},.0,'(l2:y1:t;y4:true;),.1A1?{t]5"
  "}'(l2:y1:f;y5:false;),.1A1?{f]5}'(y2:u8),.1v?{'0,${.4,.8,:(i10)^[02}E2"
  "]5}'(y2:s8),.1v?{'1,${.4,.8,:(i10)^[02}E2]5}'(y3:u16),.1v?{'2,${.4,.8,"
  ":(i10)^[02}E2]5}'(y3:s16),.1v?{'3,${.4,.8,:(i10)^[02}E2]5}'(y3:u32),.1"
  "v?{'4,${.4,.8,:(i10)^[02}E2]5}'(y3:s32),.1v?{'5,${.4,.8,:(i10)^[02}E2]"
  "5}'(y3:u64),.1v?{'6,${.4,.8,:(i10)^[02}E2]5}'(y3:s64),.1v?{'7,${.4,.8,"
  ":(i10)^[02}E2]5}'(y3:f32),.1v?{'(i10),${.4,.8,:(i10)^[02}E2]5}'(y3:f64"
  "),.1v?{'(i11),${.4,.8,:(i10)^[02}E2]5}'(y3:c64),.1v?{'(i14),${.4,.8,:("
  "i10)^[02}E2]5}'(y4:c128),.1v?{'(i15),${.4,.8,:(i10)^[02}E2]5}.1,'(s23:"
  "unexpected name after #),.6,@(y10:read-error)[53}'(c&),.1C=?{.2R0${.4,"
  ":3^[01}b]3}'(c;),.1C=?{.2R0${.4,:3^[01}.2,:9^[31}'(c|),.1C=?{.2R0${,#0"
  ".5,.1,&2{%0:1R0,.0R8?{'(s25:end of file in #| comment),:1,@(y10:read-e"
  "rror)[12}'(c|),.1C=?{:1R1,.0R8?{'(s25:end of file in #| comment),:1,@("
  "y10:read-error)[22}'(c#),.1C=?{:1R0]2}:0^[20}'(c#),.1C=?{:1R1,.0R8?{'("
  "s25:end of file in #| comment),:1,@(y10:read-error)[22}'(c|),.1C=?{:1R"
  "0${:0^[00}:0^[20}:0^[20}:0^[10}.!0.0^_1[00}.2,:9^[31}'(c(),.1C=?{.2R0$"
  "{f,:8^,.6,.5,:7^[04}X1]3}'(c%5c),.1C=?{.2R0.2R1,.0R8?{'(s20:end of fil"
  "e after #%5c),.4,@(y10:read-error)[42}.0,'(cx)C=?{.3R0${.5R1,:5^[01}?{"
  ...
  "eturn),.1v?{'(c%0d)]6}.1,'(s15:unknown #%5c name),.7,@(y10:read-error)"
  "[63}.3R0.0]4}.0C5?{:0?{${'(s44:#N=/#N# notation is not allowed in this"
  " mode),.5,@(y10:read-error)[02}}n,,#0.4,.1,:4,:3,:2,:1,&6{%1:5R0,.0R8?"
  "{'(s32:end of file within a #N notation),:5,@(y10:read-error)[22}.0C5?"
  "{.1,.1c,:4^[21}'(c#),.1C=?{.1A9X3,'(i10),.1E9,.0I0?{:0^,.1A3}{f},.0?{."
  "0d]5}.2,'(s22:unknown #n# reference:),:5,@(y10:read-error)[53}'(c=),.1"
  "C=?{.1A9X3,'(i10),.1E9,.0I0~?{${.3,'(s22:invalid #n= reference:),:5,@("
  "y10:read-error)[03}}{:0^,.1A3?{${.2,'(s18:duplicate #n= tag:),:5,@(y10"
  ":read-error)[03}}{f}}fb,:0^,${.3,:1^[01},.3cc:!0${:5,:2^[01},${.2,:3^["
  "01}?{.3,'(s31:#n= has another label as target),:5,@(y10:read-error)[63"
  "}.0,.2sz.0]6}'(s34:invalid terminator for #N notation),:5,@(y10:read-e"
  "rror)[22}.!0.0^_1[31}.0,'(s16:unknown # syntax),.4,@(y10:read-error)[3"
  "3}.0,'(s22:illegal character read),.3,@(y10:read-error)[23}.!(i18).(i1"
  "8),.(i14),.(i18),.(i15),&4{%4${.3,:3^[01},:0^,.1q?{'(s42:missing car -"
  "- ( immediately followed by .),.3,@(y10:read-error)[52}.0,,#0.0,.5,:3,"
  ":2,.(i10),:1,.(i11),:0,&8{%1.0R8?{'(s41:eof inside list -- unbalanced "
  "parentheses),:6,@(y10:read-error)[12}:1,.1q?{n]1}:0^,.1q?{:3?{${:6,:2^"
  "[01},${:6,:5^[01},:1,.1q?{.1]3}.0,'(s31:randomness after form after do"
  "t),:6,@(y10:read-error)[33}'(s13:dot in #(...)),:6,@(y10:read-error)[1"
  "2}${.2,:4^[01}?{.0d,'(s20:error inside list --),:6,@(y10:read-error)[1"
  "3}${${:6,:5^[01},:7^[01},.1c]1}.!0.0^_1[51}.!(i19).(i13),.(i11),.(i20)"
  ",&3{%2'(l6:y3:u32;y3:s32;y3:u64;y3:s64;y3:c64;y4:c128;),.2A0?{@(y9:opt"
  "-tower)~}{f}?{${@(y4:name),'(s33:unsupported numerical vector type),.4"
  ",@(y10:read-error)[03}}'(c(),.1R0q~?{${${.5,'(s23:invalid ~avector syn"
  "tax),@(y6:format)[02},.3,@(y10:read-error)[02}}${.2,:0^[01},,#0.3,.3,:"
  "0,.3,:2,:1,&6{%1.0R8?{${:5,'(s19:eof inside ~avector),@(y6:format)[02}"
  ",:4,@(y10:read-error)[12}:0^,.1q?{n]1}${.2,:1^[01}?{.0d,${:5,'(s24:err"
  "or inside ~avector --),@(y6:format)[02},:4,@(y10:read-error)[13}'(y2:u"
  "8),:5q?{.0I0?{'(i255),.1,,'0I>!;I>!}{f}}{f},.0?{.0}{'(y2:s8),:5q?{.1I0"
  "?{'(i127),.2,,'(i-128)I>!;I>!}{f}}{f},.0?{.0}{'(y3:u16),:5q?{.2I0?{'(i"
  "65535),.3,,'0I>!;I>!}{f}}{f},.0?{.0}{'(y3:s16),:5q?{.3I0?{'(i32767),.4"
  ",,'(i-32768)I>!;I>!}{f}}{f},.0?{.0}{'(y3:u32),:5q?{.4Nw!?{'0,.5<!?{'(i"
  "32),.5G8I>!}{f}}{f}}{f},.0?{.0}{'(y3:s32),:5q?{.5Nw!?{'(i32),.6G8I<}{f"
  ...
  "{.(i10)Tj}{f},.0?{.0}{'(y4:c128),:5q?{.(i11)Tj}{f}}_1}_1}_1}_1}_1}_1}_"
  "1}_1}_1}_1}_1?{${${:4,:3^[01},:2^[01},.1c]1}.0,${:5,:5,'(s29:invalid ~"
  "a inside ~avector --),@(y6:format)[03},:4,@(y10:read-error)[13}.!0.0^_"
  "1[21}.!(i20).(i22),&1{%2.0R0,.0R8?{${.4,'(s20:end of file within a),.5"
  ",@(y10:read-error)[03}}'(c%5c),.1C=,.0?{.0}{'(c%22),.2C=,.0?{.0}{'(c|)"
  ",.3C=}_1}_1?{.0]3}'(ca),.1C=?{'(c%07)]3}'(cb),.1C=?{'(c%08)]3}'(ct),.1"
  "C=?{'(c%09)]3}'(cn),.1C=?{'(c%0a)]3}'(cv),.1C=?{'(c%0b)]3}'(cf),.1C=?{"
  "'(c%0c)]3}'(cr),.1C=?{'(c%0d)]3}'(cx),.1C=?{t,.2,:0^[32}'(y6:string),."
  "3q?{.0C1}{f}?{.1R1,'(c%0a),.2C=,,#0.0,.5,&2{%2.1R8,.0?{.0}{.2C1~}_1?{."
  "0?{f]2}'(s32:no newline in line ending escape),:0,@(y10:read-error)[22"
  "}.0?{'(c%0a),.2C=}{f}?{f]2}:0R0:0R1,.1,.0?{.0}{'(c%0a),.4C=}_1,:1^[22}"
  ".!0.0^_1[32}.0,'(y1::),.4,'(s22:invalid char escape in),.5,@(y10:read-"
  "error)[35}.!(i21).7,.(i16),.(i16),.(i11),&4{%2,#0:3,.2,&2{%1.0u?{'(s31"
  ":%5cx escape sequence is too short),:0,@(y10:read-error)[12}'(i16),.1A"
  "9X3X7,:1^,.1>?{'(s27:%5cx escape sequence overflow),:0,@(y10:read-erro"
  "r)[22}.0X9]2}.!0'0,n,.3R1,,#0.0,.6,:0,:1,.8,.(i11),:2,&7{%3.0R8?{:1?{'"
  "(s27:end of file within a string),:5,@(y10:read-error)[32}.1,:2^[31}:1"
  "?{'(c;),.1C=}{f}?{:5R0.1,:2^[31}:1~?{${.2,:0^[01}}{f}?{.1,:2^[31}${.2,"
  ":3^[01}~?{.0,'(s37:unexpected char in %5cx escape sequence),:5,@(y10:r"
  "ead-error)[33}:4^,.3>?{'(s30:%5cx escape sequence is too long),:5,@(y1"
  "0:read-error)[32}:5R0'1,.3+,.2,.2c,:5R1,:6^[33}.!0.0^_1[33}.!(i22)${.("
