...
and `regp`, and scans the half-open range between them.

`r` stays pinned at the base of the file. `r[0]`–`r[5]` are the VM's shadow
registers and everything above them is its stack, so the range handed to the
collector runs from the base to the current stack pointer: `hp_reserve(n)` inside an
instruction passes `r` and `sp`, and a hand-coded function passes `r` and the `sp` it
was called with — which is why each one documents itself as protecting the registers
from `r` to `sp`. Anything a function needs to survive a collection therefore has to
live in a register or be pushed below `sp`, never held only in a C local.
