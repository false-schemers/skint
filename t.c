...
char *t_code[] = {
  ...
  "U2,.0V3,.2Zi,:2,.1,.3V4,.4A3,.0?{.0d}{f}_1W5:2W6]4}${.2,'(l2:y4:load;y"
  ...
  ":sexp-match?)[02}?{${@(y14:print-version!)[00}@(y20:enhanced-tty-libra"
  "ry)?{${@(y20:enhanced-tty-library),'(s26:enhanced-tty library: ~a~%25)"
  ",t,@(y6:format)[03}}Zh,Zb,Zc,'(s49:~d collections, ~d reallocs, heap s"
  "ize ~d words~%25),t,@(y6:format)[15}${.2,'(l1:y2:gc;),@(y11:sexp-match"
  "?)[02}?{Zg'(l1:y2:si;),:1^[11}${.2,'(l1:y4:help;),@(y11:sexp-match?)[0"
  "2}?{:2,'(s57:%0aREPL commands (,load ,cd ,sh arguments need no quotes)"
  ":%0a)W4:2,'(s44: ,load <fname>       load <fname> into REPL%0a)W4:2,'("
  "s59: ,q                  quiet: disable informational messages%0a)W4:2"
  ",'(s51: ,q-                 enable informational messages%0a)W4:2,'(s3"
  "9: ,v                  turn verbosity on%0a)W4:2,'(s40: ,v-           "
  "      turn verbosity off%0a)W4:2,'(s48: ,ref <name>         show denot"
  "ation for <name>%0a)W4:2,'(s45: ,rnr                show root name reg"
  "istry%0a)W4:2,'(s50: ,rref <name>        lookup name in root registry%"
  "0a)W4:2,'(s52: ,rrem! <name>       remove name from root registry%0a)W"
  "4:2,'(s45: ,unr                show user name registry%0a)W4:2,'(s50: "
  ",uref <name>        lookup name in user registry%0a)W4:2,'(s52: ,urem!"
  " <name>       remove name from user registry%0a)W4:2,'(s46: ,gs       "
  "          show global store (big!)%0a)W4:2,'(s55: ,gs <name>          "
  "lookup global location for <name>%0a)W4:2,'(s56: ,time <expr>        t"
  "ime single-line expression <expr>%0a)W4:2,'(s60: ,pwd                s"
  "how skint's current working directory%0a)W4:2,'(s62: ,cd <dir>        "
  "   change skint's current working directory%0a)W4:2,'(s51: ,sh <cmdlin"
  "e>       send <cmdline> to local shell%0a)W4:2,'(s41: ,si             "
  "    display system info%0a)W4:2,'(s55: ,gc                 force gc to"
  " finalize lost objects%0a)W4:2,'(s31: ,help               this help%0a"
  ")W4]1}${.2,'(l1:y1:h;),@(y11:sexp-match?)[02}?{'(l1:y4:help;),:1^[11}:"
  "2,'(s29:syntax error in repl command%0a)W4:2,'(s37:type ,help to see a"
  "vailable commands%0a)W4]1}.!0.0^_1[41",
