...
char *t_code[] = {
  ...
  "P", "repl-trace",
  "%4${.2,'(l2:y5:skint;y5:trace;),@(y20:repl-require-library)[02}.3u?{.0"
  ",'(s12:no names to )W4.0,.2W4.0W6]4}.3,,#0.2,.5,.2,.8,.7,&5{%1.0u?{:4,"
  "@(y16:repl-environment),:1,:0c,@(y22:repl-evaluate-top-form)[13}.0aY0?"
  "{.0d,:2^[11}:4,'(s9:invalid ,)W4:4,:3W4:4,'(s11: argument: )W4:4,.1aW5"
  ":4W6]1}.!0.0^_1[41",
  ...
  "2:pp;),@(y11:sexp-match?)[02}?{:0^,:2,@(y17:repl-pretty-print)[12}${.2"
  ",'(l2:y2:pp;y1:*;),@(y11:sexp-match?)[02}?{:0^,:2,@(y17:repl-pretty-pr"
  "int)[12}${.2,'(l1:y2:da;),@(y11:sexp-match?)[02}?{:0^,:2,@(y11:repl-di"
  "sasm)[12}${.2,'(l2:y2:da;y1:*;),@(y11:sexp-match?)[02}?{:0^,:2,@(y11:r"
  "epl-disasm)[12}${.2,'(l1:y1:v;),@(y11:sexp-match?)[02}?{t@!(y9:*verbos"
  "e*)'(s17:verbosity is on~%25),t,@(y6:format)[12}${.2,'(l1:y2:v-;),@(y1"
  "1:sexp-match?)[02}?{f@!(y9:*verbose*)'(s18:verbosity is off~%25),t,@(y"
  "6:format)[12}${.2,'(l1:y1:q;),@(y11:sexp-match?)[02}?{t@!(y7:*quiet*)'"
  "(s13:quiet is on~%25),t,@(y6:format)[12}${.2,'(l1:y2:q-;),@(y11:sexp-m"
  "atch?)[02}?{f@!(y7:*quiet*)'(s14:quiet is off~%25),t,@(y6:format)[12}$"
  "{.2,'(l2:y4:time;y1:*;),@(y11:sexp-match?)[02}?{Z3,${:2,@(y16:repl-env"
  "ironment),:0^a,@(y22:repl-evaluate-top-form)[03}Z4,.1,Z3-/,'(i1000)*,'"
  "(s24:; elapsed time: ~s ms.~%25),t,@(y6:format)[23}${.2,'(l1:y3:pwd;),"
  "@(y11:sexp-match?)[02}?{:2,${@(y17:current-directory)[00}W4:2W6]1}${.2"
  ",'(l2:y2:cd;y8:<string>;),@(y11:sexp-match?)[02}?{:0^a,@(y17:current-d"
  "irectory)[11}${.2,'(l2:y2:sh;y8:<string>;),@(y11:sexp-match?)[02}?{:0^"
  "aZ6]1}${.2,'(l1:y2:si;),@(y11:sexp-match?)[02}?{${@(y14:print-version!"
  ")[00}@(y20:enhanced-tty-library)?{${@(y20:enhanced-tty-library),'(s26:"
  "enhanced-tty library: ~a~%25),t,@(y6:format)[03}}Zh,Zb,Zc,'(s49:~d col"
  "lections, ~d reallocs, heap size ~d words~%25),t,@(y6:format)[15}${.2,"
  "'(l1:y2:gc;),@(y11:sexp-match?)[02}?{Zg'(l1:y2:si;),:1^[11}${.2,'(l1:y"
  "4:help;),@(y11:sexp-match?)[02}?{:2,'(s57:%0aREPL commands (,load ,cd "
  ",sh arguments need no quotes):%0a)W4:2,'(s44: ,load <fname>       load"
  " <fname> into REPL%0a)W4:2,'(s71: ,im <lib> ...       import libraries"
  ": 1 is (srfi 1), fx is (skint fx)%0a)W4:2,'(s68: ,tr <name> ...      t"
  "race named procedures, fetching (skint trace)%0a)W4:2,'(s39: ,untr <na"
  "me> ...    stop tracing them%0a)W4:2,'(s65: ,pp <expr>          pretty"
  "-print <expr>, fetching (skint print)%0a)W4:2,'(s65: ,da <proc>       "
  ...
  "          turn verbosity on%0a)W4:2,'(s40: ,v-                 turn ve"
  "rbosity off%0a)W4:2,'(s48: ,ref <name>         show denotation for <na"
  "me>%0a)W4:2,'(s45: ,rnr                show root name registry%0a)W4:2"
  ",'(s50: ,rref <name>        lookup name in root registry%0a)W4:2,'(s52"
  ": ,rrem! <name>       remove name from root registry%0a)W4:2,'(s45: ,u"
  "nr                show user name registry%0a)W4:2,'(s50: ,uref <name> "
  "       lookup name in user registry%0a)W4:2,'(s52: ,urem! <name>      "
  " remove name from user registry%0a)W4:2,'(s46: ,gs                 sho"
  "w global store (big!)%0a)W4:2,'(s55: ,gs <name>          lookup global"
  " location for <name>%0a)W4:2,'(s56: ,time <expr>        time single-li"
  "ne expression <expr>%0a)W4:2,'(s60: ,pwd                show skint's c"
  "urrent working directory%0a)W4:2,'(s62: ,cd <dir>           change ski"
  "nt's current working directory%0a)W4:2,'(s51: ,sh <cmdline>       send"
  " <cmdline> to local shell%0a)W4:2,'(s41: ,si                 display s"
  "ystem info%0a)W4:2,'(s55: ,gc                 force gc to finalize los"
  "t objects%0a)W4:2,'(s31: ,help               this help%0a)W4]1}${.2,'("
  "l1:y1:h;),@(y11:sexp-match?)[02}?{'(l1:y4:help;),:1^[11}:2,'(s29:synta"
  "x error in repl command%0a)W4:2,'(s37:type ,help to see available comm"
  "ands%0a)W4]1}.!0.0^_1[41",
